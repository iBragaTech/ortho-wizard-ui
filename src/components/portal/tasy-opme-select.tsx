...
export type TasyOpmeItem = {
  quantidade?: number;
  codigo: string;
  nome: string;
  grupo: string | null;
};
export function TasyOpmeSelect({
  value,
  onChange,
}: {
  value: TasyOpmeItem[];
  onChange: (items: TasyOpmeItem[]) => void;
}) {
  ...
  const query = useQuery({
    queryKey: ["tasy", "opme", search, offset],
    enabled: search !== null,
    retry: false,
    queryFn: () =>
      getTasyClient().executeOperation<{ items: TasyOpmeItem[]; hasMore: boolean }>(
        "catalogos.opme",
        { busca: search, offset },
      ),
  });
  function find() {
    ...
    if (search === draft.trim() && offset === 0) void query.refetch();
  }
  function toggle(item: TasyOpmeItem) {
    const alreadySelected = value.some((v) => v.codigo === item.codigo);
    onChange(alreadySelected ? value.filter((v) => v.codigo !== item.codigo) : [...value, item]);
    if (!alreadySelected) {
      setDraft("");
      setSearch(null);
      setOffset(0);
  ...
  return (
    <div className="grid gap-2">
      <div ref={searchArea} className="flex gap-2">
        <Input
          aria-label="Pesquisar OPME no Tasy"
      ...
      {query.isFetching && <p role="status">Consultando OPME…</p>}
          ...
          <div className="max-h-56 overflow-y-auto rounded-md border p-2">
            {query.data.items.map((item) => (
              <label key={item.codigo} className="flex items-start gap-2 py-2">
                <input
                  type="checkbox"
                  checked={value.some((v) => v.codigo === item.codigo)}
