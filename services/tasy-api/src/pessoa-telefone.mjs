...
export function phoneOperation({ directDmlEnabled, selectPerson, canAccess, preparePersonWrite }) {
  return {
    ...
    schema: z.object({
      ...
      telefone: z.string().trim().max(40).regex(/^[+\d ().-]+$/)
    ...
    execute: async ({ connection, input, principal }) => {
      if (!directDmlEnabled)
        throw new ApiError(501, "MAPPING_NOT_APPROVED", "Atualização de telefone no Tasy não habilitada.");
      await preparePersonWrite(connection, principal);
      const result = await connection.execute(`${selectPerson} FOR UPDATE WAIT 5`,
        { cdPessoaFisica: input.cdPessoaFisica }, { maxRows: 2 });
      if (result.rows?.length !== 1)
        throw new ApiError(409, "PATIENT_CHANGED", "Consulte novamente o paciente no Tasy.");
      const person = result.rows[0];
      if (person.nrCpf !== input.nrCpf)
        throw new ApiError(409, "PATIENT_CHANGED", "O CPF do paciente foi alterado. Consulte novamente.");
      ...
      if (current === digits) return { cdPessoaFisica: input.cdPessoaFisica, telefone: input.telefone };
      if (current !== phoneDigits(input.anterior))
        throw new ApiError(409, "RECORD_CHANGED", "O telefone foi alterado no Tasy. Consulte novamente o paciente antes de salvar.");
      const update = await connection.execute(`UPDATE TASY.pessoa_fisica
        ...
        telefone: digits.slice(2), ddd: digits.slice(0, 2), ddi: "55",
        nmUsuario: principal.tasyUsername, cdPessoaFisica: input.cdPessoaFisica,
      }, { autoCommit: false });
