...
test("request lifecycle enforces roles, ownership, history, stale edits and inactive state", async (t) => {
  ...
  for (const [i, perfil] of ["Administrador", "Comercial", "Médico", "Médico"].entries()) {
    const row = (
      await db.query(
        "INSERT INTO portal.users(nome,email,perfil,password_hash) VALUES ($1,$2,$3,$4) RETURNING id",
        [`User${i}`, `${i}@example.test`, perfil, "unused"],
      )
    ).rows[0];
    users.push({ ...row, perfil });
  }
  ...
  const create = async () =>
    (
      await db.query(
        `INSERT INTO portal.requests(numero,created_by,assigned_to,status,data) VALUES (gen_random_uuid()::text,$1,$1,'aguardando_medico',$2::jsonb) RETURNING id`,
        [doctor.id, JSON.stringify({ paciente: { telefone: "123" }, observacoes: "old" })],
      )
    ).rows[0].id;
  const id = await create();
  await assert.rejects(
    run("deleteRequest", { id, motivo: "test" }, commercial),
    (e) => e.code === "FORBIDDEN",
  );
  await assert.rejects(
    run("deactivateRequest", { id, motivo: "test" }, other),
    (e) => e.code === "NOT_FOUND",
  );
  const edit = {
    id,
    telefone: "456",
    observacoes: "new",
    anterior: { telefone: "123", observacoes: "old" },
  };
  await assert.rejects(run("editRequest", edit, doctor), (e) => e.code === "FORBIDDEN");
  await run("editRequest", edit, commercial);
  ...
  for (const operation of ["deleteRequest", "deactivateRequest"])
    await assert.rejects(
      run(operation, { id: frozen, motivo: "test" }, admin),
      (e) => e.code === "EXPORT_FROZEN",
    );
  await assert.rejects(
    run("editRequest", { ...edit, id: frozen }, admin),
    (e) => e.code === "EXPORT_FROZEN",
  );
});
